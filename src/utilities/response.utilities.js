...
// modules
import { ErrorResponse, SuccessResponse } from '../schema'
...
export const success = success_obj => {
  try {
    ...
    success_obj = validate_schema(success_obj, SuccessResponse)

    const { status, message, data } = success_obj

    if (status === 200) return ok(message, data)
    if (status === 201) return created(message, data)
    if (status === 204) return no_content(message, data)

    console.assert(!success_obj)
...
export const throw_error = error => {
  // validate error schema
  error = validate_schema(error, ErrorResponse)
  ...
  if (status === 500) error = internal_server(message)
